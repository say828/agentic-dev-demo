...
> **측정 방법(정직).** 시간 = git 커밋 타임스탬프 앵커 기반(작업 종료 시각). 토큰 =
> 에이전트가 세션 내부에서 직접 읽을 수 없음 → Claude Code `/cost` 누적값을 받아 기록한다.
> `/cost` 는 세션 누적치이므로, "구간 토큰" = (현재 누적) − (직전 기록 누적)으로 계산한다.

| 작업 | 커밋 | 종료 시각(KST) | 소요(앵커 기반) | 토큰(`/cost`) |
| --- | --- | --- | --- | --- |
| pinterest 데모 | `f56e419` | 21:02:59 | — (세션 시작 시각 미기록) | 미기록 — `/cost` 필요 |
| cyworld 데모 | `089d3dd` | 21:11:49 | 약 9분 (pinterest 커밋→cyworld 커밋, 브레인스토밍 포함) | 미기록 — `/cost` 필요 |

- 다음 작업부터는 **시작 시 누적 `/cost` 를 먼저 기록**해 구간 소요·토큰을 정확히 산출한다.
- 토큰 값은 `/cost` 출력을 알려주면 위 표의 해당 칸을 채운다(추정치 임의 기입 안 함).
