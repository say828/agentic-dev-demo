...
그래서 플래닝 단계만 보면 눈에 보이는 화면이 아직 없습니다.

여기서 확정한 계획(AC-1~AC-6)은 뒤 단계(02_plan → 03_build → 04_verify)를 거쳐
실제로 동작하는 회원가입 OTP 화면이 됩니다. 이 `complete` 프로젝트가 그 완성 상태입니다.
...
- 화면 오른쪽 패널에는 이 기획(AC-1~AC-6)과 SDD 산출물 체인이 함께 표시됩니다.
- 완성 화면 미리보기는 `sdd/04_verify/02_screen/assets/`의 캡처(step1~step3)에서 확인합니다.

> 정리하면, 기획 문서가 마지막에 이 화면 한 장으로 이어집니다. 그 연결을 미리 알고
...
| screen · signup_otp 화면 | `02_screen/signup_otp_screen.md` | 구현·검증 완료 |
